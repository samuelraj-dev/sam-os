...
//     // before the asm jump
//     SystemTable->ConOut->OutputString(SystemTable->ConOut, L"pml4 addr: ");
//     print_hex(SystemTable, (UINT64)boot_pml4);
//     SystemTable->ConOut->OutputString(SystemTable->ConOut, L"\r\n");
//     SystemTable->ConOut->OutputString(SystemTable->ConOut, L"pdpt addr: ");
//     print_hex(SystemTable, (UINT64)boot_pdpt_low);
//     SystemTable->ConOut->OutputString(SystemTable->ConOut, L"\r\n");
...
efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
    // ── Allocate page tables at known low physical address ────
    // MUST be first — before any other allocation
    // 32 pages = enough for pml4 + pdpt + pd_hi + 28 PDs for identity map
    EFI_PHYSICAL_ADDRESS pt_base = 0x600000;
    EFI_STATUS pt_status = SystemTable->BootServices->AllocatePages(
        AllocateAddress, EfiLoaderData, 32, &pt_base);
    if (EFI_ERROR(pt_status)) {
        SystemTable->ConOut->OutputString(SystemTable->ConOut, L"[ERROR] PT alloc failed\r\n");
        while(1);
    }

    // zero all 32 pages
    for (int i = 0; i < 512 * 32; i++)
        ((uint64_t*)pt_base)[i] = 0;

    uint64_t* boot_pml4     = (uint64_t*)(pt_base + 0x0000);  // page 0
    uint64_t* boot_pdpt_low = (uint64_t*)(pt_base + 0x1000);  // page 1
    uint64_t* boot_pdpt_hi  = (uint64_t*)(pt_base + 0x2000);  // page 2
    uint64_t* boot_pd_hi    = (uint64_t*)(pt_base + 0x3000);  // page 3
    // pages 4-31 = up to 28 PDs for identity map (28GB coverage)
    ...
    // ── Build Page Tables ────────────────────────────────────
    // dynamically map enough GB to cover framebuffer
    UINT64 fb_end = (UINT64)gop->Mode->FrameBufferBase +
                    gop->Mode->Info->VerticalResolution *
                    gop->Mode->Info->PixelsPerScanLine * 4;

    // round up to next GB boundary, add 1GB margin
    UINT64 map_top = (fb_end + 0x3FFFFFFF) & ~0x3FFFFFFFULL;
    map_top += 0x40000000ULL;  // 1GB margin

    UINT64 num_pdpt = map_top / 0x40000000ULL;
    if (num_pdpt > 28) num_pdpt = 28;  // max 28 PDs available

    SystemTable->ConOut->OutputString(SystemTable->ConOut, L"[INFO] Mapping ");
    print_hex(SystemTable, num_pdpt);
    SystemTable->ConOut->OutputString(SystemTable->ConOut, L" GB identity\r\n");

    boot_pml4[0] = (UINT64)boot_pdpt_low | 0x3;

    for (UINT64 p = 0; p < num_pdpt; p++) {
        // PDs start at page 4 (offset 0x4000 from pt_base)
        uint64_t* pd = (uint64_t*)(pt_base + 0x4000 + p * 0x1000);
        boot_pdpt_low[p] = (UINT64)pd | 0x3;
        for (int i = 0; i < 512; i++)
            pd[i] = (p * 0x40000000ULL + (UINT64)i * 0x200000) | 0x83;
    }
    ...
    SystemTable->ConOut->OutputString(SystemTable->ConOut, L"[INFO] pml4: ");
    print_hex(SystemTable, pml4_addr);
    SystemTable->ConOut->OutputString(SystemTable->ConOut, L"\r\n");
